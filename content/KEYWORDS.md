# SEO keyword plan — indiaxchange.net

One article per day targets the next `pending` keyword below. When an article
ships, set its status to `done` and fill the slug. Keep every article honest:
no invented statistics, no fake reviews, no guarantees of profit or rankings.

Owner-provided keywords first (verbatim), then expansions. Group = articles
that can share one page (Google treats them as the same intent).

| # | Keyword | Status | Article slug |
|---|---------|--------|--------------|
| 1 | sell usdt | done | how-to-sell-usdt-for-inr |
| 2 | sell usdt for inr | done | how-to-sell-usdt-for-inr |
| 3 | how to sell usdt in india | done | how-to-sell-usdt-for-inr |
| 4 | sell usdt cheap | done | sell-usdt-best-rate-india |
| 5 | usdt to inr best rate | done | sell-usdt-best-rate-india |
| 6 | sell usdt safe | done | sell-usdt-safely-avoid-scams |
| 7 | usdt to inr safe | done | sell-usdt-safely-avoid-scams |
| 8 | safe inr funds | done | avoid-bank-freeze-when-selling-usdt |
| 9 | p2p bank account freeze | done | avoid-bank-freeze-when-selling-usdt |
| 10 | usdt to inr money | done | usdt-to-inr-bank-transfer-explained |
| 11 | usdt to inr bank transfer | done | usdt-to-inr-bank-transfer-explained |
| 12 | usdt to upi | done | usdt-to-inr-bank-transfer-explained |
| 13 | usdt selling india | done | usdt-selling-india-complete-guide |
| 14 | sell tether in india | done | usdt-selling-india-complete-guide |
| 15 | binance p2p alternative india | done | binance-p2p-alternative-india |
| 16 | sell usdt without binance | done | binance-p2p-alternative-india |
| 17 | clean funds | done | sell-usdt-clean-funds |
| 18 | sell usdt clean funds | done | sell-usdt-clean-funds |
| 19 | sell usdt get clean funds | done | sell-usdt-clean-funds |
| 20 | clean inr funds | done | sell-usdt-clean-funds |
| 21 | verified indian platform | done | verified-indian-usdt-platform |
| 22 | verified usdt platform india | done | verified-indian-usdt-platform |
| 23 | sell usdt instantly | done | sell-usdt-instantly |
| 24 | usdt to imps transfer | done | sell-usdt-instantly |
| 25 | convert usdt to inr | done | convert-usdt-to-inr |
| 26 | usdt to inr | done | convert-usdt-to-inr |
| 27 | sell usdt trc20 | done | sell-usdt-trc20-vs-bep20 |
| 28 | sell usdt bep20 | done | sell-usdt-trc20-vs-bep20 |
| 29 | tether to rupees | done | convert-usdt-to-inr |
| 30 | usdt otc desk india | done | usdt-otc-desk-india |
| 31 | sell large amount usdt india | done | usdt-otc-desk-india |
| 32 | usdt p2p india | pending | |
| 33 | sell crypto for cash india | pending | |
| 34 | is it legal to sell usdt in india | pending | |
| 35 | usdt inr rate today | pending | |
| 36 | sell usdt from trust wallet | pending | |

Priority note: the owner's core conversion terms — "sell usdt", "clean
funds", "sell usdt get clean funds", "verified indian platform" — are
covered by dedicated pages AND woven into the home page title/description/
hero. Future articles should interlink to those two pages whenever the
topic touches money sourcing or trust.

Style notes for every article:
- 600–900 words, `##` sections, one `###` max per section.
- Interlink: at least 2 other /learn guides + /sell + /guarantee or a legal page.
- End with the sell CTA (the template adds it automatically).
- India-first language (rupees, UPI/IMPS, lakh/crore where natural).
- Facts only: what the desk actually does; tax/legal topics get a "talk to a
  professional" note, never advice.
